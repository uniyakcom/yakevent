...
// 特点: Emit ~15ns/op，UnsafeEmit ~3.8ns/op，~175ns/op 高并发（低核），error 返回
func Sync() *Profile {
	return &Profile{
		Name:        "sync",
		Conc:        1000,
...
// 特点: ~23ns/op 单线程，~16ns/op 高并发，零 CAS，零分配，Per-P SPSC ring
func Async() *Profile {
	return &Profile{
		Name:        "async",
		Conc:        100000,
...
// 特点: ~74ns/op 单线程，多阶段 Pipeline，per-shard 精准唤醒，自适应分片，At-least-once
func Flow() *Profile {
	return &Profile{
		Name:         "flow",
		Conc:         5000,
